...
const activeDragonSpins = new Set<string>()
...
export default {
   async getdragon(req: Request, res: Response) {
      try {
         ...
         logger.info({ event: "fortune_dragon.game_info.start", ...requestInfo(req), atk: mask(token) }, "Fortune Dragon GameInfo recebido")
         const user = await allfunctions.getuserbyatk(token);
         if (!user[0]) {
            res.status(401).send({ dt: null, err: { cd: "1302", msg: "Usuario nao encontrado." } })
            return false
         }
         ...
         if (!jsoninicial[0]) {
            res.status(500).send({ dt: null, err: { cd: "1500", msg: "Json inicial nao encontrado para fortune-dragon." } })
            return false
         }
         ...
         logger.error({ event: "fortune_dragon.game_info.error", error }, "Erro no GameInfo Fortune Dragon")
         res.status(500).send({ dt: null, err: { cd: "1500", msg: "Erro interno no GameInfo." } })
      }
   },
   async spin(req: Request, res: Response) {
      ...
      try {
         ...
         if (activeDragonSpins.has(token)) {
            res.status(409).send({ dt: null, err: { cd: "1309", msg: "Spin em andamento." } })
            return false
         }
         activeDragonSpins.add(token)
         let bet: number = cs * ml * 5
         let saldoatual: number = Number(user[0].saldo || 0)
         const gamename = "fortune-dragon"
         ...
         const checkuserbalance = /^https?:\/\//i.test(callbackUrl)
            ? await axios({
            ...
            : { data: { msg: "SUCCESS", user_balance: saldoatual } }
         ...
         logger.error({ event: "fortune_dragon.spin.error", error }, "Erro no Spin Fortune Dragon")
         res.status(500).send({ dt: null, err: { cd: "1500", msg: "Erro interno no Spin." } })
      } finally {
         activeDragonSpins.delete(token)
      }
   },
}
